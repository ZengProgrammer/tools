...
**Goal:** 新增时间戳 ↔ 日期时间双向转换工具，纯前端，默认实时时钟模式。

**Architecture:** 新建 TimestampView.tsx 组件，照搬 JsonView/SqlView 的双列布局模式。左侧秒级时间戳（可编辑），右侧日期时间+四种精度时间戳（只读）。用 `setInterval` 驱动每秒流逝，`Intl.DateTimeFormat` 格式化时区。

**Tech Stack:** React 18, Fluent UI v9, JavaScript Date API

---

### Task 1: 创建 TimestampView.tsx
...
**Commit:**
```bash
git add src/views/TimestampView.tsx
git commit -m "feat: add TimestampView with live clock and bidirectional conversion"
```

---

### Task 2: 修改 HomeView、Sidebar、AppLayout、FloatingWindow 四个文件

**Files:**
- Modify: `src/views/HomeView.tsx` — 加首页卡片
- Modify: `src/components/Sidebar.tsx` — 加导航项
- Modify: `src/components/AppLayout.tsx` — 导入 + 始终挂载
- Modify: `src/views/FloatingWindow.tsx` — 加悬浮窗标签

### 2a. HomeView.tsx — 加首页卡片

在 tools 数组末尾添加（找个合适的图标，比如 `TimerRegular`）：
...
需要 import `ClockRegular` from `@fluentui/react-icons`。
...
需要 import `ClockRegular`。

### 2c. AppLayout.tsx — 导入 + 始终挂载
...
在 main 内添加挂载 div（在 SqlView div 之后）：
```tsx
...
同时更新 activeKey 相关逻辑支持 'timestamp'。

### 2e. Commit

```bash
git add -A
git commit -m "feat: integrate TimestampView into home, sidebar, floating window, and app layout"
```
...
- [ ] 运行 `pnpm run build`，确认 TypeScript 零错误 + Vite 构建成功
- [ ] 如有编译错误，修复后提交
